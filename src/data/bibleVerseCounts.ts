...
export const BIBLE_CHAPTER_VERSE_COUNTS: Record<string, number[]> = {
  ...
  // Epistles & Revelation (መልእክቶችና ራእይ)
...
export function getExpectedVerseCount(bookId: string, chapter: number): number {
  const counts = BIBLE_CHAPTER_VERSE_COUNTS[bookId.toUpperCase()];
  if (!counts || chapter < 1 || chapter > counts.length) {
    return 20; // safe default estimate
  }
